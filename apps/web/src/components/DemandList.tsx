...
export function DemandList({ demands, selectedDemandId, onSelectDemand, source, sseStatus, lastEventLabel, onReload }: DemandListProps) {
  return (
    <aside className="w-full border-b border-white/10 bg-black/20 p-4 backdrop-blur-xl xl:w-[360px] xl:border-b-0 xl:border-r">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs text-white/55">{demands.length}</span>
      </div>

      <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.025] p-3 text-xs text-white/50">
        <div className="flex items-center justify-between gap-3">
          <span>Fonte: <span className={source === "api" ? "text-emerald-300" : "text-yellow-300"}>{source === "api" ? "API" : "mock"}</span></span>
          <span>SSE: <span className={sseStatus === "connected" ? "text-emerald-300" : sseStatus === "connecting" ? "text-blue-300" : "text-red-300"}>{sseStatus}</span></span>
        </div>
        {lastEventLabel && <div className="mt-2 text-white/35">Último evento: {lastEventLabel}</div>}
        <button onClick={onReload} className="mt-3 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-white/65 transition hover:bg-white/[0.08]">Recarregar</button>
      </div>
      ...
      <div className="mt-4 space-y-3">
        {demands.map((demand) => {
          ...
          return (
            <button key={demand.id} onClick={() => onSelectDemand(demand.id)} className={`w-full rounded-2xl border p-4 text-left transition hover:bg-white/[0.06] ${selected ? "border-emerald-400/35 bg-emerald-400/10" : "border-white/10 bg-white/[0.025]"}`}>
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`rounded-full border px-2 py-1 text-[10px] ${demandStatusClass[demand.status]}`}>{demandStatusLabel[demand.status]}</span>
              </div>
