...
export async function workflowRunRoutes(app: FastifyInstance): Promise<void> {
  app.get("/", async () => prisma.workflowRun.findMany({ orderBy: { createdAt: "desc" }, include: { demand: { select: { id: true, title: true, project: true } } } }));

  app.get("/latest", async (request, reply) => {
    const query = z.object({ project: z.string().optional() }).parse(request.query);
    const workflowRun = await prisma.workflowRun.findFirst({ where: { project: query.project }, orderBy: { createdAt: "desc" } });
  ...
  app.post("/", async (request, reply) => {
    ...
    const workflowRun = await prisma.workflowRun.create({ data: { demandId: body.demandId, project: body.project, title: body.title, status: body.status, summary: body.summary, payload: JSON.parse(JSON.stringify(body.payload)) } });
